  ...
  override func provideThumbnail(for request: QLFileThumbnailRequest, _ handler: @escaping (QLThumbnailReply?, Error?) -> Void) {
    ...
    if FileManager.default.isReadableFile(atPath: request.fileURL.path) {
      ...
      do {
        ...
        let data = try handle.read(upToCount: BUFFOON_CONSTANTS.THUMBNAIL_BYTE_COUNT) ?? Data()
        ...
        let lines: [Substring] = codeFileString.split(maxSplits: BUFFOON_CONSTANTS.THUMBNAIL_LINE_COUNT + 1, omittingEmptySubsequences: false, whereSeparator: { $0.isNewline })
